...
import { getMe, getPatient } from "@/lib/api";
import type { Patient } from "@/types";
import { Pill, Activity, Microscope, ArrowUpRight, Zap } from "lucide-react";
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {medications.length > 0 ? (
          medications.map((med, i) => (
            <div
              key={i}
              className={`${cardStyle} p-8 flex flex-col justify-between group hover:bg-white`}
            >
          ...
          <>
            {/* Empty State */}
            <div
              className={`${cardStyle} p-8 flex flex-col items-center justify-center text-center bg-slate-50/50 border-dashed border-slate-200`}
            >
              ...
              <p className="text-[10px] text-slate-400 uppercase tracking-widest font-bold max-w-xs leading-relaxed">
                Clinical protocols are added by your healthcare provider
                following analysis.
              </p>
            </div>
            ...
            >
              ...
              <p className="text-[10px] text-slate-400 uppercase tracking-widest font-bold max-w-xs leading-relaxed">
                Additional medications will appear here when prescribed.
              </p>
            </div>
          </>
        )}
        ...
        {/* Intelligence Module */}
        <div
          className={`${cardStyle} md:col-span-2 p-10 bg-slate-900 text-white border-slate-800 flex items-center gap-10`}
        >
          ...
          <div>
            ...
            <p className="text-sm font-medium text-slate-300 leading-relaxed max-w-2xl">
              Healynx assists clinical decision-making by analyzing molecular
              docking pathways. All listed medications are monitored for
              cross-interaction in real-time.
            </p>
          </div>
